...
LRESULT APIENTRY Engine::SubclassProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {

	HRAWINPUT hRawInput;
	PRAWINPUT pRawInput;
	RID_DEVICE_INFO sRidDeviceInfo;
	unsigned long dwKeystate;	
	...
	case WM_HSCROLL:
		/*if (selectedNode != 0) {
			temprot.FromAngleAxis(Radian(Math::PI/45),rot*Vector3(0.0,1.0,0.0));
			selectedNode->rotate(temprot,Node::TS_PARENT);
		}*/
		break;
	case WM_CLOSE:
		DestroyWindow(hwnd);
	...
	case WM_INPUT: 
		...
		if (appMode == spaceNavigator) {

			hRawInput = reinterpret_cast<HRAWINPUT>(lParam);

			if (::GetRawInputData(hRawInput, RID_INPUT, 0, &dwSize, sizeof(RAWINPUTHEADER)) == 0 )
				saRawInput.reset(new BYTE[dwSize]);

			pRawInput = reinterpret_cast<PRAWINPUT>(saRawInput.get());
			if (!pRawInput) break;
...
#define PADDING 100
						...
						cursor.modelMatrix[3][0] += X/PADDING;
						cursor.modelMatrix[3][2] += Y/PADDING;
						cursor.modelMatrix[3][1] += Z/PADDING;	
					}
					...
					else if (pRawInput->data.hid.bRawData[0] == 0x02)  
					{
						...
						//cursorNode->rotate(axis,norm,Ogre::Node::TS_WORLD);

					}
					//Buttons
					else if (pRawInput->data.hid.bRawData[0] == 0x03)
					{
						dwKeystate = *reinterpret_cast<unsigned long *>(&pRawInput->data.hid.bRawData[1]);
					}
				}
			}
	...
	//return CallWindowProc(g_ogreWinProc, hwnd, msg, wParam, lParam);
}
