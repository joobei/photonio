...
public:
  ...
  */
  ...
  /*!
    ...
    \param in_Ec is the energy per channel symbol
    \param in_N0 is the single-sided power spectral density of the AWGN on the channel.
  */
  ...
  */
  void map_decode(const vec &rec_systematic, const mat &rec_parity, const vec &extrinsic_input, vec &extrinsic_output, 
  ...
  */
  void log_decode(const vec &rec_systematic, const mat &rec_parity, const vec &extrinsic_input, vec &extrinsic_output,
  ...
  /*!
    ...
    \param metric May be "LOGMAP" or "LOGMAX" (default)
  */
  void log_decode_n2(const vec &rec_systematic, const vec &rec_parity, const vec &extrinsic_input, vec &extrinsic_output, 
		     bool in_terminated = false, string metric = "LOGMAX");

private:

  //! Returns a forward state transition in the trellis
  void state_transition(int s_prim, int input_bit, int &s, int &parity);
  //! Returns a backward state transition in the trellis
  void reverse_state_transition(int s, int input_bit, int &s_prim, int &parity);
