...
/*! 
  \file 
  \brief Recursive Systematic Convolutional Codes (Rec_Syst_Conv_Code)
...
void Rec_Syst_Conv_Code::state_transition(int s_prim, int input_bit, int &s, int &parity) 
{ 
  s = state_trans(s_prim,input_bit); 
  parity = output_parity(s_prim,input_bit); 
}

void Rec_Syst_Conv_Code::reverse_state_transition(int s, int input_bit, int &s_prim, int &parity) 
{
  s_prim = rev_state_trans(s,input_bit); 
  parity = rev_output_parity(s,input_bit);
}
		      ...
		      vec &extrinsic_output, bool in_terminated)
{
  ...
  //Initiate beta
  if (terminated) {
    beta.set_col( block_length, zerovec ); 
		      ...
		      vec &extrinsic_output, bool in_terminated, string metric)
{
  ...
  for (k=block_length; k>=1; k--) { 
    ...
    for (l=0; l<Nstates; l++) { beta(l,k) -= denom(k); }
  }
		      ...
		      vec &extrinsic_output, bool in_terminated, string metric)
{
  ...
  //Check that Lc = 1.0
  //  it_assert(Lc==1.0,
  //	    "Rec_Syst_Conv_Code::logmap_decode: This function assumes that Lc = 1.0. Please use proper scaling of the input data");
