...
{
    Convolutional_Code::encode(input, output);
...
{
  int nn = 0, i = 0, p, j, total = 0;

  for (j=0; j<n; j++) {
    for (p=0; p<Period; p++)
      total += (int)puncture_matrix(j,p);
  }
    
  int temp_size = (int)ceil(((double)received_signal.size()*(double)Period)/(double)total) * n;
  vec temp(temp_size);
  j = 0;
  p = 0;
  ...
  Convolutional_Code::decode(temp, output);
}
...
{
  int state = 0, zero_state, one_state, zero_temp, one_temp, i, j, p = 0, prev_pos = 0, no_symbols, total=0;
  int block_length = 0;
...
void Punctured_Convolutional_Code::distance_profile(llvec &dist_prof, int start_time, int dmax, bool reverse)
{
  int max_stack_size = 50000;
...
int Punctured_Convolutional_Code::fast(Array<llvec> &spectrum, int start_time, int dfree, int no_terms, int d_best_so_far, bool test_catastrophic)
{
...
void Punctured_Convolutional_Code::calculate_spectrum(Array<llvec> &spectrum, int dmax, int no_terms)
{
...
void Punctured_Convolutional_Code::calculate_spectrum(Array<llvec> &spectrum, int time, int dmax, int no_terms, int block_length)
{
