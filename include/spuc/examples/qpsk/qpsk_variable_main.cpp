...
*/
// $Id: qpsk_variable_main.cpp,v 1.2 2005/09/16 17:04:55 spuc Exp $
#include <stdio.h>
#include <stdlib.h>
...
{                        
  ...
  do {
	...
	sampf << Environment.RECEIVER->adc_out.re << ' ';
	sampf << Environment.agc_scale << '\n';
#endif

	if (Environment.RECEIVER->symbol_locked != QPSK_symbol_locked) {
	  if (QPSK_symbol_locked) {
		printf("Loss of symbol lock at symbol %d\n",Environment.rcv_symbols);
	  } else {
		printf("Symbol lock declared at symbol %d\n",Environment.rcv_symbols);
	  }
	...
	if (Environment.RECEIVER->symclk()==1) {
	  ...
	  // DEBUG		
		...
		sampf << (Environment.RECEIVER->symbol_loop_out) << ' ';
		sampf << re(Environment.RECEIVER->final_baseband) << ' ';
		sampf << '\n';
#endif
